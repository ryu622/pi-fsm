...
Method: generate synthetic populations of "arrival points" under four
...
the resulting Vmax(delta_t) curves against the REAL curve (R):
  ...
  constant_noise   (C) -- "measurement-error fingerprint"
  timevarying_no_noise (T') -- "pure model-limitation fingerprint"
  timevarying_noise    (T) -- both effects combined
...
(documents/phase2_pilot_results.md), integrated forward under the
ORIGINAL Fujimura-Sugihara assumption of an arbitrary driving-force
direction n per instance (this is what makes Baseline 1's circular
arrival region meaningful -- PINN's own training fixed n=(1,0) for a
different, simpler purpose, but the underlying F(t),k(t) magnitudes are
direction-agnostic physical quantities and can be reused here).
...
DT_INTEGRATE = 0.002
NOISE_STD = 1.0  # meters -- TRACAB's stated accuracy (documents/research_plan.md)
...
def main() -> None:
    ...
    results = [
        run_condition("constant_no_noise", F_const, k_const, noise_std=0.0),
        run_condition("constant_noise", F_const, k_const, noise_std=NOISE_STD),
        run_condition("timevarying_no_noise", F_net, k_net, noise_std=0.0),
        run_condition("timevarying_noise", F_net, k_net, noise_std=NOISE_STD),
    ]
    ...
    styles = {
        "constant_no_noise": dict(color="#898781", ls="--", label="定数・ノイズなし(パイプライン確認用)"),
        "constant_noise": dict(color="#eb6834", ls="-", label="C: 定数+ノイズ(測定誤差の指紋)"),
        "timevarying_no_noise": dict(color="#1baf7a", ls="-", label="T': 時間変化のみ・ノイズなし(モデル限界の指紋)"),
        "timevarying_noise": dict(color="#2a78d6", ls="-", label="T: 時間変化+ノイズ"),
    }
    for cond, style in styles.items():
        sub = combined[combined["condition"] == cond].sort_values("delta_t")
        ax.plot(sub["delta_t"], sub["vmax"], "o-", **style)
    real_dt = sorted(REAL_VMAX.keys())
    ax.plot(real_dt, [REAL_VMAX[d] for d in real_dt], "ks--", lw=2, ms=8, label="R: 実データ(J03WPY, フェーズ1)")
    ax.set_xlabel("Δt [s]")
    ax.set_ylabel("$V_{max}$")
    ax.legend(fontsize=8, loc="upper right")
    ax.set_title("フェーズ4: 実データの不安定性はどちらの指紋に近いか")
    fig.tight_layout()
