...
**2段階学習**: ①$\hat x(t,v_0)$ をデータ損失のみで学習・重み固定 → ②固定した$\hat x$に対し、$F(t),k(t)$のネットワークのみを物理損失で学習。$\hat x$と$F,k$の共適応(動くターゲットを同時に追いかける不安定性)を切り離すことで、識別性が改善するか確認する。素朴な線形回帰による事後フィットとは異なり、$F,k$は引き続き物理損失で学習されるニューラルネットの出力であり、PINNとしての性質は保たれる。
